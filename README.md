...
# Prédiction des Débits Extrêmes du Fleuve Mono par Approche Hybride (STL-GEV-DL)
...
L'intégralité de la recherche est regroupée dans le fichier principal :
*   **`Nom_de_ton_fichier.ipynb`** : Ce notebook unique couvre tout le pipeline, du prétraitement des données brutes à l'évaluation finale des performances (KGE, Rappel, F1-Score) et l'analyse des seuils POT (Peak Over Threshold).
